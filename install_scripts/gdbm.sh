...
PKG_NAME="gdbm"
REPO_URL="http://git.gnu.org.ua/cgit/gdbm.git"
...
VERFILE=""
INSTALLED_VERSION=
if hash gdbmtool 2>/dev/null ;then
  INSTALLED_VERSION=$(gdbmtool -V | head -n1 | cut -d' ' -f3)
fi
...
if ([ ! -z $REINSTALL ] && [ $LEVEL -le $REINSTALL ]) || [ -z $INSTALLED_VERSION ] || $(compare_version $INSTALLED_VERSION $VER); then
  ...
  curl --retry 10 -L ftp://ftp.gnu.org/gnu/gdbm/gdbm-$VER.tar.gz | tar xz && cd $FOLDER
