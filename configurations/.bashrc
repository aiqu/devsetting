...
MY_PATH=$GOPATH/bin:${LOCAL_DIR}/bin:$HOME/bin:$PATH

[[ ! $PATH == *"$MY_PATH"* ]] && export PATH=${MY_PATH}${PATH:+":$PATH"}
[[ ! $CPATH == *"$MY_INCLUDE_DIR"* ]] && export CPATH=${MY_INCLUDE_DIR}${CPATH:+":$CPATH"}
[[ ! $LD_LIBRARY_PATH == *"$MY_LIBRARY_DIR"* ]] && export LD_LIBRARY_PATH=${MY_LIBRARY_DIR}:${SYSTEM_LIBRARY_DIR}${LD_LIBRARY_PATH:+":$LD_LIBRARY_PATH"}
[[ ! $LIBRARY_PATH == *"$MY_LIBRARY_DIR"* ]] && export LIBRARY_PATH=${MY_LIBRARY_DIR}${LIBRARY_PATH:+":$LIBRARY_PATH"}
[[ ! $CMAKE_LIBRARY_PATH == *"$LD_LIBRARY_PATH"* ]] && export CMAKE_LIBRARY_PATH=${LD_LIBRARY_PATH}${CMAKE_LIBRARY_PATH:+":$CMAKE_LIBRARY_PATH"}
[[ ! $CMAKE_INCLUDE_PATH == *"$CPATH"* ]] && export CMAKE_INCLUDE_PATH=${CPATH}${CMAKE_INCLUDE_PATH:+":$CMAKE_INCLUDE_PATH"}
[[ ! $PKG_CONFIG_PATH == *"$MY_PKG_CONFIG_DIR"* ]] && export PKG_CONFIG_PATH=${MY_PKG_CONFIG_DIR}${PKG_CONFIG_PATH:+":$PKG_CONFIG_PATH"}
[[ ! $MANPATH == *"MY_MANPATH"* ]] && export MANPATH=${MY_MANPATH}${MANPATH:+":$MANPATH"}
